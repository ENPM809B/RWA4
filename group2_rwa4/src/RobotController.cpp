#include <RobotController.h>
  ...
  * @return
  */
RobotControl::RobotControl(std::string armid) :
  robot_controller_nh_("/ariac/"+armid),
  robot_controller_options("manipulator",
        "/ariac/"+armid+"/robot_description",
        robot_controller_nh_),
  robot_move_group_(robot_controller_options) {
    ROS_WARN("<<<<< RobotController >>>>>");
      robot_move_group_.setPlanningTime(60);
      robot_move_group_.setNumPlanningAttempts(40);
      ...
      // home_joint_pose_ = {0.0, 3.14, -1.57, 0.0, 4.54, -1.63, 0.0};
...
void RobotControl::GoToTarget(geometry_msgs::Pose& pose) {
  // pose.position.z = pose.position.z + offset_;
